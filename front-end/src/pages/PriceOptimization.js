...
export default function PriceOptimization({ sellers, storeDB, socket, expressAPI }) {
    ...
    useEffect(() => {
        socket.emit('PO:getData', { sellers: sellers,  token:localStorage.getItem('loginJWTToken')})
    ...
    function scrapePrices() {
        socket.emit('PO:startScraper', { scraperDatas: sellers.map(seller => { return { scraper: seller, mode: 1 } }) ,  token:localStorage.getItem('loginJWTToken')})
    }
