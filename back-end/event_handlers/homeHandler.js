...
module.exports = (io, socket, getCeleryID, activeTasks) => {
    ...
    socket.on('home:stopScraper', data => {
    ...
    socket.on('home:getData', async (data) => {
        let graphData = {}
        ...
        socket.emit('home:updateInfo', { graphData: graphData, itemData: nullItems, seller: data.seller })

    });

    socket.on('home:startScraper', async (scraperData) => {
    ...
    client.on('notification', async function (msg) {
        ...
        socket.emit('home:updateInfo', { graphData: graphData, itemData: nullItems, seller: seller })
    })
