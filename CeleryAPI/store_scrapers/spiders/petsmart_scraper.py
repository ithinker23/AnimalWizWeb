...
class PetSmartScraperSpider(scrapy.Spider):
        ...
        if(self.updateMode == 1):
            ...
            for id in cur.fetchall():
                ...
                for res in cur.fetchall():
                    ...
                    yield SeleniumRequest(url = res[1], callback=self.parse_product,errback=self.closeSpider, cb_kwargs={"pid":res[0], "id":id[0], 'pages':int(cfg['petsmart']['s_pages'])}, script=sel_script)
            ...
            for result in query_results:
                yield scrapy.Request(url=self.base_url + self.search_url + self.sanitizeQuery(result[1]), callback=self.parse,errback=self.closeSpider, cb_kwargs={"pid":result[0], "id":None, 'pages':int(cfg['petsmart']['s_pages'])})
        else:
            #SCRAPE MANUAL ENTRY
            yield scrapy.Request(url=self.url, callback=self.parse_product,errback=self.closeSpider, cb_kwargs={"pid":self.pid, "id":None, 'pages':int(cfg['petsmart']['s_pages'])})
    ...
    def parse(self, response, pid, id, pages):
        ...
        if(next_page != []):
            yield response.follow(next_page, callback=self.parse,errback=self.closeSpider, cb_kwargs={"pid":pid, "pages":pages, 'id':id})
